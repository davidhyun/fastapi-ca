...
이용자별로 각자 매일 새로 배운 지식을 게시하고 관리할 수 있는 서비스

## 1. 개발 컨셉

### 1.1. 도메인 주도 개발 (Domain Driven Develop)

클린 아키텍처는 계층형 아키텍처에서 발전된 형태이다. 계층을 완전히 별개로 보면 모든 통신을 인터페이스로 해야 하고, 구조가 너무 복잡해진다.

| 계층            | 설명                                                         |
| --------------- | ------------------------------------------------------------ |
| **Domain**      | 핵심 비즈니스 로직, 도메인 객체, Repository 인터페이스       |
...
-H 'Content-Type: application/json' \
-d '{
    "name": "김수한무거북이와 두루미 삼천갑자 동방삭 가나다라마바사아자차카타파하가나다라마바사아자차카나파하가나다라마바사아자차카타파하",
    "email": "normal_string",
    "password": "짧은비번"
}'
